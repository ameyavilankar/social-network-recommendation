...
USER_BIAS_FILE = "facebook.bias.U.1_of_1"
ITEM_BIAS_FILE = "facebook.bias.V.1_of_1"
...
def calcTrainAUC():
    ...
    user_bias = get_data(USER_BIAS_FILE, ",")
    item_bias = get_data(ITEM_BIAS_FILE, ",")
    ...
    for row in user_bias:
        user_bias_map[row[0]] = row[1]

    for row in item_bias:
        item_bias_map[row[0]] = row[1]
    ...
    for i in range(train_data.shape[0]):
        y_scores[i] = np.dot(user_vectors[train_data[i,0]], item_vectors[train_data[i, 1]]) + user_bias_map[train_data[i,0]] + item_bias_map[train_data[i, 1]] + 1
